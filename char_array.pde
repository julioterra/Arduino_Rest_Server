...
// check whether it is a space or '/' variable
  ...
  Serial.print("[next_element] END seaching for next_element in: ");
  Serial.print(_msg_source);
  Serial.print(" start: ");
  Serial.print(start);
  Serial.print(" end: ");
...
int return_number (char* _msg_source, int start) {
	...
	if (_msg_source[start] == '/' || _msg_source[start] == '=' ) {
		start += 1;
	}
    ...
    if (end_index == -1 && end_index != start) { 
		end_index = master_length - 1;
    } else if (end_index == start) {
        start += 1;
        return return_number(_msg_source, start);      
    } else { 
		if (_msg_source[end_index] == '/' || _msg_source[end_index] == ' ') end_index -= 1; 
	}
   
    return convert_string2int(_msg_source, start, end_index);
}
...
void slice (char* _msg_source, int _start_index, int _end_index) {
	...
	if (_start_index > master_length || _start_index < 0 || 
		_start_index > _end_index || _end_index < 0) {
		return;
	}

    for (int i = 0; i < master_length; i++) {
        int shift_start_index = _start_index + i;
        if (shift_start_index < master_length || shift_start_index < _end_index) { 
